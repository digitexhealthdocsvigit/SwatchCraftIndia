...
const ProductWaterfall: React.FC<Props> = ({ onNavigate, onDownload }) => {
  ...
  const thumbnails = [
    { label: "Main View", src: "https://images.unsplash.com/photo-1534944209481-ffccd2b13b91?auto=format&fit=crop&w=800&q=80" },
    { label: "Bi-Fold", src: "https://images.unsplash.com/photo-1523366952726-2615433d7d74?auto=format&fit=crop&w=800&q=80" },
    { label: "Tri-Fold", src: "https://images.unsplash.com/photo-1505569127510-bde15360d7f6?auto=format&fit=crop&w=800&q=80" },
    { label: "Pallete", src: "https://images.unsplash.com/photo-1519681393784-d120267933ba?auto=format&fit=crop&w=800&q=80" }
  ];
      ...
      <section className="py-12 px-4 lg:py-20 max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-start">
        <div className="space-y-6">
          <div className="aspect-[4/3] bg-gray-50 rounded-[3rem] overflow-hidden border border-gray-100 shadow-2xl relative group">
            <img src={thumbnails[activeThumb].src} className="w-full h-full object-cover transition-all duration-700 group-hover:scale-110" />
            <div className="absolute top-8 left-8 bg-teal text-white text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg">#1 Exporter Choice</div>
          </div>
          <div className="grid grid-cols-4 gap-4">
            {thumbnails.map((thumb, idx) => (
              <button key={idx} onClick={() => setActiveThumb(idx)} className={`aspect-square rounded-2xl border-2 overflow-hidden transition-all ${activeThumb === idx ? 'border-gold scale-95 shadow-inner' : 'border-transparent opacity-60'}`}>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
             <div className="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-100">
                <p className="text-[9px] text-gray-400 font-black uppercase mb-1">Starting from</p>
                <p className="text-3xl font-black text-navy leading-none">₹70 <span className="text-[10px] text-gray-400 uppercase font-bold">/ unit</span></p>
             </div>
             <div className="bg-gray-50 p-8 rounded-[2.5rem] border border-gray-100 flex flex-col justify-center">
                <p className="text-[9px] text-gray-400 font-black uppercase mb-1">Standard Lead Time</p>
          ...
          <div className="mt-12 grid grid-cols-3 gap-8">
             <div className="flex flex-col items-center">
                ...
                <span className="text-[8px] font-black text-gray-400 uppercase">Space Optimized</span>
             </div>
             <div className="flex flex-col items-center">
                <Zap className="w-6 h-6 text-gold mb-2" />
                <span className="text-[8px] font-black text-gray-400 uppercase">Rapid Sample</span>
             </div>
             <div className="flex flex-col items-center">
                <TrendingUp className="w-6 h-6 text-gold mb-2" />
                <span className="text-[8px] font-black text-gray-400 uppercase">Sales Focused</span>
             </div>
          </div>
        </div>
      </section>
              ...
              <tbody className="divide-y divide-gray-50">
                {waterfallSpecs.map((row, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    <td className="p-6 font-black text-navy text-xs uppercase tracking-widest bg-gray-50/50 w-1/3">{row.s}</td>
                    <td className="p-6 text-gray-500 font-bold text-sm">{row.d}</td>
                  </tr>
                ))}
              </tbody>
            </table>
