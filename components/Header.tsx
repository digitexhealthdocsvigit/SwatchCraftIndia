...
const Header: React.FC<HeaderProps> = ({ onNavigate, currentView }) => {
  ...
  const scrollToSection = (id: string) => {
    if (currentView !== 'home') {
      onNavigate('home');
      // Delay scrolling until after state update might be needed, but simple router usually works
      setTimeout(() => {
  ...
  const navItems = [
    ...
    { name: 'Products', id: 'products', type: 'section' },
    { name: 'Process', id: 'process', type: 'section' },
    { name: 'Export Markets', id: 'export-markets', type: 'section' },
    ...
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled || mobileMenuOpen ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
        ...
        <div className="flex flex-col cursor-pointer" onClick={() => onNavigate('home')}>
          <span className={`text-xl md:text-2xl font-bold tracking-tight text-navy`}>
            SwatchCraft <span className="text-gold">India</span>
          </span>
        ...
        <nav className="hidden lg:flex items-center space-x-6">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => {
                if (item.type === 'view') onNavigate(item.id as 'home' | 'about');
                else scrollToSection(item.id);
              }}
              className={`text-sm font-semibold transition-colors ${
                (currentView === item.id) ? 'text-gold' : 'text-navy hover:text-gold'
              }`}
            >
              {item.name}
            </button>
        ...
        <div className="lg:hidden bg-white border-t border-gray-100 absolute top-full w-full shadow-xl">
          <div className="p-4 space-y-4">
            {navItems.map((item) => (
              <button
                ...
                className={`block w-full text-left py-2 text-lg font-medium border-b border-gray-50 ${
                  currentView === item.id ? 'text-gold' : 'text-navy'
                }`}
              >
                {item.name}
              </button>
            ...
            <div className="pt-4 flex flex-col space-y-3">
              ...
              <a href="mailto:info@swatchcraftindia.com" className="bg-teal text-white text-center font-bold py-3 rounded">Email RFQ</a>
            </div>
          </div>
        </div>
      )}
